...
def verifyResults(results: str,matches: pd.DataFrame,teams: pd.DataFrame)-> bool | str:
    ...
    try:
        ...
        assert result[0] in teams['TeamName'].unique(),"First Team not found"
        assert result[1] in teams['TeamName'].unique(),"Second Team not found"
        assert result[0] != result[1],"A team can't play against itself!"
        assert int(result[2])
        assert int(result[3])
        return True
    except ValueError as e:
